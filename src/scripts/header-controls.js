...
function createThemeToggle() {
  ...
  containers.forEach(containerId => {
    ...
    switchThumb.className = `absolute top-0.5 left-0.5 w-4 h-4 rounded-full transition-all duration-300 shadow-md ${
      isDark ? 'translate-x-5 bg-gray-800' : 'translate-x-0 bg-white'
    }`;
    ...
    checkbox.addEventListener('change', function() {
      ...
      switchThumb.className = `absolute top-0.5 left-0.5 w-4 h-4 rounded-full transition-all duration-300 shadow-md ${
        newIsDark ? 'translate-x-5 bg-gray-800' : 'translate-x-0 bg-white'
      }`;
